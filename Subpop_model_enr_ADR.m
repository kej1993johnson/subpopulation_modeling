...
pat = xlsread('../data/subpop_sorting_extraADR.xls');
...
Vmaxall =[];
for j = 1:nsamp-1
    Vmaxbymix(j) = mean(Vmax(j:j+2));
    Vmaxmat = repmat(Vmaxbymix(j), ns.*nreps,1);
    Vmaxall = vertcat(Vmaxall, Vmaxmat);
end
% special case for last mixture nreps = 6
Vmaxbymix(5) = mean(Vmax(5:end));
Vmaxmat = repmat(Vmaxbymix(5), ns.*6,1);
Vmaxall = vertcat(Vmaxall,Vmaxmat);

%%
params0 = horzcat( [225 0.1 24 0.1], 0.5.*ones(1, nsamp));%  LD50res, sloperes, LD50sens, slopesens, fres
...
options = optimset('Display','off','FunValCheck','on', ...
    ...
    ihi = find(pat(:,7) ==1); % 1st sample is pure ADR
    ilo = find(pat(:,7) == 4); % 4th sample is pure WT
    psingle0low = [ 70 0.1];
    psingle0high = [350 0.1];
...
[Phi, resnormhi, residualshi] = lsqnonlin(@fitsinglepop,...
    ...
    dose(ihi),...
    viability(ihi));
[Plo, resnorm, residuals] = lsqnonlin(@fitsinglepop,...
    psingle0high,...
    paramslb(1:2),...
    paramsub(1:2),...
    options,...
    ...
    viability(ilo));
%%
[P, resnorm, residuals] = lsqnonlin(@fitmixedpops_ADRenr,...
    params0,...
    paramslb,...
    paramsub,...
    options,...
...
[lowerlim, upperlim] = finderrorBS_ADRenr(residuals,P, dose, nsamp, nreps, Vmaxbymix, Vmaxall);

Pactual = [ 0.07 0.28 0.51 0.75 0.96];

stddevs = [ 0.004 0.022 0.017 0.017 0.0019];
CImeas = stddevs*1.96;
%%
switch nsamp
    case 4 %6/30/17 samples
        ...
        save('../out/param_table4_ADR.mat', 'param_table')
...
scatter(dose(ns*(5)+1:ns*6), viability(ns*(5)+1:ns*6), Color{5}, 'LineWidth', 1)
...
title ('b', 'FontSize', 18)
legend('0%', '25%','50%','75%','100%', 'line of unity')
legend boxoff
title ('b', 'FontSize', 18)
